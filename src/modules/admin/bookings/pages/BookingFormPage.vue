    ...
    <div v-else class="bg-white dark:bg-gray-900 shadow rounded-lg">
      ...
      <form @submit.prevent="handleSubmit" class="px-4 py-5 sm:px-6 space-y-6 border-t border-gray-200 dark:border-gray-700">
        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
          <FormField :label="m.adminBookingsUi.userIdOptional">
            <FormInput
          ...
          <FormField :label="m.adminBookingsUi.vehicleId">
            <FormInput
        ...
        <div class="flex gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
          ...
          >
            ...
            {{ saving ? m.adminBookingsUi.saving : m.adminBookingsUi.saveChanges }}
          </button>
        </div>
      </form>
    </div>
...
const handleSubmit = async () => {
  try {
    ...
    toastError(e?.response?.data?.message || m.value.adminBookingsUi.errorSaving)
  } finally {
    saving.value = false
  }
}
