<template>
  <Modal :show="true" @close="handleCancel">
    <template #header>
      <h2 class="text-lg font-bold text-[var(--app-text)]">{{ m.adminRolesUi.deleteTitle }}</h2>
    </template>

    <p class="text-[var(--app-muted-text)] p-5 text-base">
    ...
    <template #footer>
      <div class="flex justify-end gap-3">
        <button
          @click="handleCancel"
          :disabled="isDeleting"
          class="px-4 py-2 text-[var(--app-muted-text)] bg-[var(--app-surface-alt)] rounded-md hover:opacity-80 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
        >
          {{ m.commonUi.cancel }}
        </button>
        <button
